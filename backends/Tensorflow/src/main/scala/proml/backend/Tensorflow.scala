...
object Tensorflow {
  ...
  implicit val tensorInstance = new Backend[TypedTensor[Double]] {
    ...
    override def randNormal(x: TypedTensor[Double]): TypedTensor[Double] =
      Tensor.rand(x.dataType, x.shape)
    override def zero(x: TypedTensor[Double]): TypedTensor[Double] =
      Tensor.zeros(x.dataType, x.shape)
    ...
    override def sumOnShape(x: TypedTensor[Double]): Double =
      x.sum(null, false).entriesIterator.next().asInstanceOf[Double]
